...
dropEvery_rec [] _ = []
dropEvery_rec _ 0 = []
{-dropEvery_rec l n = [ x | x <- l, x /= n ]-}
{-dropEvery_rec l n = map (l!!) [0,n-1..(length l-1)] {-,n-1+n..]-} -}
{-dropEvery_rec l n = [ l !! i | i <- [n-1,n-1+n..(length l - 1)] ] -}
{-
dropEvery_rec l n = z l (f l)
	where
		f x = [ i | i <- [n-1,n-1+n..(length x - 1)] ]
		z a b = drop (head b) a
-}
{-dropEvery_rec l n = [head l] ++ [ l !! i | i <- [0..(length l - 1)], (i `mod` n) /= 0 ] -}
{- envy the lameness ^ -}
dropEvery_rec l n = [ l !! i | i <- [0..(length l - 1)], ((i+1) `mod` n) /= 0 ]


{-
dropEvery_rec_2 :: [a] -> Int -> [a]
dropEvery_rec_2 [] _ = []
dropEvery_rec_2 _ 0 = []
dropEvery_rec_2 l n = [head l] ++ [ l !! i | i <- [1..(length l - 1)], ((i+1) `mod` n) /= 0 ]
-}
...
{-
test_dropEvery_rec_2 = TestCase $ do
	assertEqual "for (dropEvery_rec_2 'abcdefghik')" "abdeghk" (take 10 (dropEvery_rec_2 "abcdefghik" 3))
	assertEqual "for (dropEvery_rec_2 [])" 0 (length (dropEvery_rec_2 [] 5))
	assertEqual "for (dropEvery_rec_2 'abc') 1" [] (dropEvery_rec_2 "abc" 1)
	assertEqual "for (dropEvery_rec_2 'abc') 0" [] (dropEvery_rec_2 "abc" 0)
	-}
...
p16_tests = TestList [
				...
				{-TestLabel "test_dropEvery_rec_2" test_dropEvery_rec_2-}
			]
